...
function htmlize(snippet){
    
    if(snippet.type == undefined){
        return snippet.text
    }
    
    const entity = {type: snippet.type, url: snippet.url}
...
function breakToSnippets(msgObj){
    ...
    const firstSnippet = { 
        ...
        type: undefined, 
        url: undefined,
        text: msgObj.text.substring(0, entities[0].offset)
    }

    snippets.push(firstSnippet)

    for(let i=0; i < entities.length; i++){
        if(i > 0 && entities[i].offset-(entities[i-1].offset + entities[i-1].length) > 0){
            snippets.push({ 
                offset: entities[i-1].offset + entities[i-1].length, 
                length: entities[i].offset,
                type: undefined, 
                url: undefined,
                text: msgObj.text.substring(entities[i-1].offset + entities[i-1].length, entities[i].offset)
            })
        }
        snippets.push({ 
            ...
            url: entities[i].url ? entities[i].url : undefined,
            text: msgObj.text.substring(entities[i].offset, entities[i].offset+entities[i].length)
        })
    }

    const endSnippet = { 
        offset: entities[entities.length-1].offset+entities[entities.length-1].length,
        length: msgObj.text.length,
        type: undefined, 
        url: undefined,
        text: msgObj.text.substring(entities[entities.length-1].offset+entities[entities.length-1].length, msgObj.text.length)
    }
    
    if(endSnippet.text.length > 0)
        snippets.push(endSnippet)

    return snippets
}

function parseToHTML(msgObj){
    let snippets = breakToSnippets(msgObj)

    let newSnippets = []
    let formattedText = ''

    for(let i=0; i < snippets.length; i++){
        const currSnippet = snippets[i]

        if(currSnippet.type == undefined){
            newSnippets.push(currSnippet)
        } else {
            let isExists = false

            for(let n=0; n < newSnippets.length; n++){
                const oldSnippet = newSnippets[n]
                if(oldSnippet.offset == currSnippet.offset && oldSnippet.length == currSnippet.length){
                    newSnippets[i] = {
                        offset: currSnippet.offset,
                        length: currSnippet.length,
                        type: currSnippet.type,
                        url: currSnippet.url,
                        text: htmlize(oldSnippet)
                    }
                    isExists = true
                }
            }
    ...
    if(newSnippets.length > 0){
        newSnippets = _(newSnippets).reverse().value()
        newSnippets = _.uniqBy(newSnippets, 'offset')
        newSnippets = _(newSnippets).reverse().value()
    }

    for(let i=0; i < newSnippets.length; i++){
        formattedText += htmlize(newSnippets[i]) //newSnippets[i].text
    }
